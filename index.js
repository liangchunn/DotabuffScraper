    ...
    request(this.URLWrapper, function(error, response, body) {
      if (error || response.statusCode !== 200) {
        ...
        process.stdout.write(
          `Trying again [${self.tries}/${self.maxTries}] \n`
        );
        return self.fetch();
      }
      ...
      $("tbody tr").each(function() {
        if (!$("td", this).hasClass("talent-cell")) {
          var newObject = {};
          var advantagePercentage = Number(
            $("td:nth-child(3)", this).attr("data-value")
          );
          var name = $("td a", this)
            .attr("href")
            .replace("/heroes/", "");
  ...
  for (var property in this.data) {
    if (index > 1) {
      if (this.data[property].length !== lengthCheck) {
        process.stdout.write(
          `[ERR!] Something went wrong with validating ${
            this.heroes[index - 1]
          } \n`
        );
        process.stdout.write(
          `[ERR!] Expected length of ${lengthCheck} but instead got ${
            this.data[property].length
          } \n`
        );
        this.dumpFile();
        return;
...
Scraper.prototype.start = function() {
  ...
  request(
    {
      url: `http://www.dotabuff.com/heroes/abaddon/matchups?date=patch_${
        self.options.patchVersion
      }`,
      headers: {
        "User-Agent":
          "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/525.13 (KHTML, like Gecko) Chrome/0.A.B.C Safari/525.13"
...
(function() {
  fs.writeFile(options.filename, "", function(error) {
    if (error) {
      process.stdout.write(
        "[ERR!] Write test failed. Verify if folder permissions are set correctly. \n"
      );
      return;
    }
    process.stdout.write("Write test successful! \n");
    process.stdout.write(
      `Fetching data for game revision ${options.patchVersion} \n`
    );
